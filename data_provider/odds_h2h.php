    ...
    if($res->success){
        foreach ($res->data as $it){
            ...
            if(mysqli_num_rows($result1)==0){
                $sql = "INSERT INTO odds (game_id, sport_key, sport_nice, home_team, away_team, team1, team2, commence_time, site_key, site_nice, last_update, h2h, ts)
                VALUES ('$game_id','$sport_key','$sport_nice', '$home_team', '$away_team' ,'$team1','$team2','$commence_time', '$site_key', '$site_nice','$last_update','$h2h', CURRENT_TIMESTAMP )";
            }else{
                $sql = "UPDATE odds SET h2h='$h2h', last_update='$last_update', commence_time='$commence_time' WHERE game_id='$game_id'";
            }
            $result = mysqli_query($connect, $sql);
        }
    }
}
print_r('Successfully finished(h2h)');
